...
//        InputStream inputStream = this.getClass().getResourceAsStream("/demo-be-0.0.1-SNAPSHOT.jar");
        InputStream inputStream = new FileInputStream(Paths.get("target/demo-be-0.0.1-SNAPSHOT.jar").toFile());

        byte[] bytes = null;
        try {
            Path path = Paths.get("test/bnc/testnet/viewer/services/Test.class");
